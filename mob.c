...
// Function to return a rand number for critcal attack
int critique(){
    return rand() % 5;
}

// Function to return a rand number for dodge
int esquive(){
    return rand() % 4;
}

// Function to fight mob and player
            ...
            if(player.hp > 0 && mobs[i].hp > 0){
                ...
                if(!critique()){ // Si le joueur fait un coup critique
                    if(!esquive()){ // Si le mob esquive le coup critique
                        ...
                        mobs[i].hp -= atk * 2; // Le mob perd 2 fois plus de vie
                    ...
                    if(!esquive()){ // Si le mob esquive le coup
                        ...
                        mobs[i].hp -= atk; // Le mob perd de la vie
                        printf("Il reste %d points de vie au mob !\r\n", mobs[i].hp);
                    }
                }
                ...
                if(mobs[i].hp <= 0){
                    ...
                    nbMob--;
                    player.xp += 100;
                    printf("Vous gagnez de l'experience !\r\n");
                    if(player.xp == 100*power(2, player.level)){
                        player.level++;
                        printf("Vous passez niveau %d !\r\n", player.level);
                    }
                }
                    ...
                    if(!esquive()){ // Si le joueur esquive le coup critique
                        ...
                        player.hp -= mobs[i].atk * 2 - res; // Le joueur perd 2 fois plus de vie
                    ...
                    if(!esquive()){ // Si le joueur esquive le coup
                        ...
                        player.hp -= mobs[i].atk - res; // Le joueur perd de la vie
                        printf("Il vous reste %d hp !\r\n", player.hp);
                    }
                }
...
// Fonction to set a mob by id
void setMob(int id, Mob mob){
    mobs[id] = mob;
}

// Funtion to alloc memory for mobs
void mallocMob(int size){
    mobs = (Mob*) malloc(size * sizeof(Mob));
}

// Function to free mobs
void freeMobs(){
    free(mobs);
}